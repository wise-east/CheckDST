    ...
    for SD in 0 1 2; do 
        for BATCH_SIZE in 8; do 
            for UPDATE_FREQ in 1; do 
                ...
                INIT_CMD="--init-model /data/home/justincho/ParlAI/data/models/bart/bart_large/model"

                LOG_DIR="/data/home/justincho/ParlAI/bash_scripts/slurm_logs/sgd_scratch_ft_lr${LR}_bs_${BATCH_SIZE}_uf_${UPDATE_FREQ}_npu${NGPU}/"
                if [[ ! -e $LOG_DIR ]]; then 
                    mkdir -p $LOG_DIR
                fi 
                ...
                INIT_CMD="--init-model /data/home/justincho/ParlAI/models/bart_g_sgd_reversed_pft/lr5e-6_eps10_ngpu8_bs8_2021-11-15_11:45:04/model"

                LOG_DIR="/data/home/justincho/ParlAI/bash_scripts/slurm_logs/sgd_pft_ft_lr${LR}_bs_${BATCH_SIZE}_uf_${UPDATE_FREQ}_npu${NGPU}/"
                if [[ ! -e $LOG_DIR ]]; then 
                    mkdir -p $LOG_DIR
                fi 
                LOG_FILE="${LOG_DIR}${NOW}.log"
