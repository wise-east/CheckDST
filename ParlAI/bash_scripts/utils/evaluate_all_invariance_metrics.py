...
for fp in fps:
    ...
    if not os.path.isfile(model_opt_fn):
        logger.error(
            f"No model.opt file found in {fp}. Training was incomplete/problematic."
        )
    ...
    if not os.path.isfile(os.path.join(fp, "model.test")):
        logger.error(
            f"No model.test file found in {fp}. Training was incomplete/problematic."
        )
        problematic_dirs.append(fp)
        continue
    ...
    for inv in invariances:
        ...
        # detect any faulty reports that were created by mistake due to wrong fewshot evaluation setting
