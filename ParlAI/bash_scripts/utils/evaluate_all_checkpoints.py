...
def eval_checkpoints_in_dir(main_dir):
    ...
    for ckpoint in fn_list:
        ...
        for split in ["valid", "test"]:
            ...
            eval_cmd = (
                f"sbatch {script_fn} -c {ckpoint} -d {split} -t False -f {is_fewshot}"
            )
            ...
            if not os.path.isfile(report_file):
                ...
                if report["report"]["joint goal acc"] < 0.1:
                    ...
                    run(shlex.split(eval_cmd))
                    logger.info(
                        f"Submitted: Evaluation results were found but previous results seem faulty: (JGA = {jga}) {eval_exec_msg}"
                    )
                else:
                    logger.info(
                        f"Skipped: Evaluation results found and previous results seem okay (JGA={jga})."
                    )
        ...
        for aug in ["NED", "SDI", "PI"]:
            ...
            eval_exec_msg = (
                f"Running evaluation with `{ckpoint}` on {aug} fewshot={is_fewshot}"
            )
            # if evaluation is not already done, run evaluation
            if not os.path.isfile(report_file):
