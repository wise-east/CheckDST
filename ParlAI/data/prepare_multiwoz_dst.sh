...
if [[ ! -e multiwoz_dst/MULTIWOZ2.1 ]]; then
    ...
    cd .. # needed due to some absolute path of mapping.pair file 
    parlai dd -t multiwoz_dst --data_version 2.1
    cd data
fi

# this will do the reformating inside the multiwoz_dst agent (refer to tasks/multiwoz_dst/utils/reformat.py)
cd .. 
parlai dd -t multiwoz_dst --data_version 2.3 
cd data 
...
if [[ ! -e multiwoz_dst/MULTIWOZ2.2 ]]; then
    ...
    cd .. # needed due to some absolute path of mapping.pair file 
    parlai dd -t multiwoz_dst --data_version 2.2
    cd data
fi
...
python check_data_across_testsets.py #this should say "All clear for $invariance"
...
## prepare few shot data for the augmented dataset 
