...
from parlai.utils.misc import Timer, nice_report, TimeLogger
...
class TrainLoop:
    ...
    # custom class for saving checkpoint
    def save_checkpoint(self):
        opt = self.opt
        checkpoint_fn = f"{opt['model_file']}.checkpoint_step{self._train_steps}"
        logging.info(
            f"saving model checkpoint: {opt['model_file']}.checkpoint_step{self._train_steps}"
        )
        self.save_model(f".checkpoint_step{self._train_steps}")
        ...
        if opt.get("optimize_for_multiwoz"):
            ...
            for eval_split in ["valid", "test"]:
                cmd = f"sbatch /data/home/justincho/ParlAI/bash_scripts/eval_checkpoint.sh -c {checkpoint_fn} -d {eval_split} -t False -f {is_fewshot}"
                logging.info(f"Executing evaluation command: {cmd}")
                subprocess.run(shlex.split(cmd))

            # submit LAUG jobs
            for aug in ["NEI", "SD", "TP"]:
                # load original few_shot configuration
                cmd = f"sbatch /data/home/justincho/ParlAI/bash_scripts/evaluate_laug_invariance.sh -m {checkpoint_fn} -i {aug} -f {is_fewshot}"
                logging.info(f"Executing LAUG invariance evaluation command: {cmd}")
                subprocess.run(shlex.split(cmd))
        return
    ...
    def train_steps(self):
        ...
        with world:
            while True:
                ...
                if (
                    save_time > self.save_every_n_secs
                    or self._total_epochs - self.last_save_epoch
                    ...
                    self.last_save_epoch = self._total_epochs
                    self._last_save_steps = self._train_steps
