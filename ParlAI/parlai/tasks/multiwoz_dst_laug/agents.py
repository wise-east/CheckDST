...
class MultiWozDSTTeacher(FixedDialogTeacher):
    ...
    def add_cmdline_args(cls, argparser, partial_opt):
        agent = argparser.add_argument_group('MultiWozDST LAUG Teacher Args')
        agent.add_argument(
            '-aug',
            '--augmentation_method',
            type=str,
    ...
    def _path(self, opt):
        ...
        data_dir = os.path.join(opt['datapath'], 'laug_dst', opt['augmentation_method'])

        # for NER invariance, load from original data and augment it dynamically
        if opt['augmentation_method'].lower() == "nei":
            data_dir = os.path.join(opt['datapath'], 'laug_dst', 'orig')
        ...
        # v23_data_path = os.path.join(opt['datapath'], 'multiwoz_dst', 'MULTIOZ2.3', 'data_reformat.json'
    ...
    def nei_augment(self, messages):
        ...
        save_path = f"/data/home/justincho/trippy-public-master/data/laug_dst/NEI/changed_dialids_{self.datatype}_{self.few_shot}.txt"
