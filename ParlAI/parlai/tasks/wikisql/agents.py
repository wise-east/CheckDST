...
class WikiSQLTeacher(DialogTeacher):
    ...
    def setup_data(self, input_path):
        ...
        for line in qa_data:
            ...
            templates = [
                f"Here's what we know about the database: {context} Translate this query into SQL: {question}",
                f"Given this information: {context} provide the SQL of this query: {question}",
                f'What is the SQL for this question "{question}" given that we have this information: "{context}"?',
                f"Here's what we want to know: {question} Generate the SQL command given that we know the following about a database we have access to: {context}",
                f'What is the SQL command we can use to get the answer to "{question}" given we know: "{context}"?',
                f'Generate the SQL query for answering the question: "{question}" given we know: {context}',
            ]

            yield (
                random.choice(templates).lower(),
                [sql.lower()],
                None,
                None,
            ), new_episode
