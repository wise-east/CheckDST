...
class MultiWozDSTTeacher(FixedDialogTeacher):
    ...
    def _setup_data(self, data_path, jsons_path):
        ...
        if self.decode_all:
            ...
            if self.val_reduced_size != -1:
                k = min(len(self.messages), self.val_reduced_size)
    ...
    def custom_evaluation(
        ...
        # print out when predictions are wrong for the test set
        if self.datatype == "test" and set(slots_truth) != set(slots_pred):
            logging.info(
