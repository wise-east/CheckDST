...
for key in ["train", "valid", "test"]:
    with open(
        os.path.join(DATAPATH, f"multiwoz_dst/MULTIWOZ2.3/data_reformat_{key}.json"),
        "r",
    ) as f:
        v23 = json.load(f)

    v22_path = os.path.join(
        DATAPATH, f"multiwoz_dst/MULTIWOZ2.2/data_reformat_{key}.json"
    )
    v21_path = os.path.join(
        DATAPATH, f"multiwoz_dst/MULTIWOZ2.1/data_reformat_{key}.json"
    )
