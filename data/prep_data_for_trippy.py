...
from utils import read_zipped_json, normalize_dialogue_ids, test_match, replace_dict_values_str
...
class NED_processor: 
    ...
    def __init__(self, original_datapath, testlist_filepath): 
        ...
        with open(testlist_filepath, "r") as f: 
            ...
            assert len(self.test_convs) == 1000, f"Number of test conversations should be 1000, but got {len(self.test_convs)}"
                            ...
                            if v == []: 
                                ...
                                for booked_places in slots['book']['booked']: 
                                    for slot_type, slot_value in booked_places.items():
                                        ...
                                        changed_outer = changed_outer or replace_with_scrambled_entity(domain, slot_type, slot_value, print_=print_)
                                        
                                
                        elif v and v!= "not mentioned":  
                            # print(f"{domain} {k} {v}")
                            # pass
                            changed_outer = changed_outer or replace_with_scrambled_entity(domain, k, v, print_=print_)
...
def examine_dialogue_acts():
    ...
    with open(os.path.join(DATADIR, "multiwoz_dst/MULTIWOZ2.3/dialogue_acts.json"), "r") as f: 
        dacts = json.load(f)
            ...
            if isinstance(acts, dict): 
                for slot, slot_vals in acts.items(): 
                    for sv in slot_vals: 
                        ...
                        if f"{slot}-{sv[0]}" == "Attraction-Inform-Name" and sv[1] != "None": 
...
if __name__ == "__main__": 
    ...
    testlist_filepath = os.path.join(DATADIR, "multiwoz_dst/MULTIWOZ2.1/testListFile.json") 
    ...
    with open(os.path.join(DATADIR, "checkdst/NED/parlai_changed_dialids_test_fewshot_False.txt"), "r") as f: 
        changed_parlai_ids = f.readlines()
        changed_parlai_ids = [c.replace("\n", "") for c in changed_parlai_ids]

    logger.info(f"Number of dial ids changed for original data.json: {len(ned_processor.change_list)}")
    logger.info(f"Number of dial ids changed for reformatted ParlAI data: {len(changed_parlai_ids)}")
     
    in_both = set(ned_processor.change_list).intersection(changed_parlai_ids) 
    logger.info(f"Only their intersection will be used to evaluate the models: {len(in_both)}") 

    with open(os.path.join(DATADIR, "checkdst/NED/both_changed_dial_ids.txt"), "w") as f: 
