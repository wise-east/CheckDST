...
class ExampleIntentBertModel(torch.nn.Module):
    def __init__(
        ...
        self.bert_model = BertModel(
            BertConfig.from_pretrained(model_name_or_path, output_attentions=True)
        )
    ...
    def forward(
        ...
        example_pooled_output = self.encode(
        ...
        intent_probs = 1e-6 + torch.zeros(
            probs.size(0), self.num_intent_labels
        ).cuda().scatter_add(
            -1, example_intents.unsqueeze(0).repeat(probs.size(0), 1), probs
        )
        ...
        if intent_label is not None:
            ...
            intent_loss = loss_fct(
                intent_lp.view(-1, self.num_intent_labels),
                intent_label.type(torch.long),
            )
        else:
            intent_loss = torch.tensor(0)
...
class SlotBertModel(torch.nn.Module):
    def __init__(self, model_name_or_path: str, dropout: float, num_slot_labels: int):
...
class JointSlotIntentBertModel(torch.nn.Module):
    ...
    def forward(
        ...
        hidden_states, pooled_output = self.bert_model(
            input_ids=input_ids,
            attention_mask=attention_mask,
            token_type_ids=token_type_ids,
        )
