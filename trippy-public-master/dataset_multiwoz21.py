...
def normalize_time(text):
    ...
    text = re.sub(
        "(^| )(\d{1,2}) (a\.?m\.?|p\.?m\.?)", r"\1\2:00 \3", text
    )  # am/pm short to long form
    text = re.sub(
    ...
    text = re.sub(
        "(^| )(at|from|by|until|after) ?(\d{1,2})([;., ]|$)", r"\1\2 \3:00\4", text
    ...
    text = re.sub(
        "(\d{2})(:\d{2}) ?p\.?m\.?",
        lambda x: str(
            int(x.groups()[0]) + 12 if int(x.groups()[0]) < 12 else int(x.groups()[0])
        )
        + x.groups()[1],
        text,
    )
    text = re.sub(
        "(^| )24:(\d{2})", r"\g<1>00:\2", text
...
def normalize_text(text):
    ...
    text = re.sub(
        "(^| )b ?& ?b([.,? ]|$)", r"\1bed and breakfast\2", text
    )  # Normalization
...
def get_token_pos(tok_list, value_label):
    ...
    found = False
    label_list = [
        item for item in map(str.strip, re.split("(\W+)", value_label)) if len(item) > 0
    ]
...
def get_turn_label(
    ...
    if (
        value_label == "none"
        or value_label == "dontcare"
        or value_label == "true"
        or value_label == "false"
    ):
...
def tokenize(utt):
    ...
    utt_tok = [
        tok for tok in map(str.strip, re.split("(\W+)", utt_lower)) if len(tok) > 0
    ]
    return utt_tok
    ...
    for dialog_id in tqdm(input_data):
        ...
        for utt in utterances:
            ...
            if usr_sys_switch == is_sys_utt:
                print(
                    "WARN: Wrong order of system and user utterances. Skipping rest of dialog %s"
                    % (dialog_id)
                )
                break
            ...
            if delexicalize_sys_utts and is_sys_utt:
                ...
                for slot in slot_list:
                    if (str(dialog_id), str(turn_itr), slot) in sys_inform_dict:
                        inform_dict[slot] = sys_inform_dict[
                            (str(dialog_id), str(turn_itr), slot)
                        ]
                utt_tok_list.append(
                    delex_utt(utt["text"], inform_dict, unk_token)
            ...
            if is_sys_utt:
                for d in utt["metadata"]:
                    ...
                    if booked != []:
                        for s in booked[0]:
                            booked_slots[s] = normalize_label(
                                "%s-%s" % (d, s), booked[0][s]
                            )  # normalize labels
                    ...
                    for category in ["book", "semi"]:
                        for s in utt["metadata"][d][category]:
                            ...
                            value_label = normalize_label(
                                cs, utt["metadata"][d][category][s]
                            )  # normalize labels
        ...
        for i in range(1, len(utt_tok_list) - 1, 2):
            ...
            for slot in slot_list:
                ...
                if (str(dialog_id), str(turn_itr), slot) in sys_inform_dict:
                    inform_label = list(
                        [
                            normalize_label(slot, i)
                            for i in sys_inform_dict[
                                (str(dialog_id), str(turn_itr), slot)
                            ]
                        ]
                    )
                ...
                if label_value_repetitions and slot in diag_seen_slots_dict:
                    if (
                        class_type == "copy_value"
                        and list(diag_seen_slots_value_dict.values()).count(value_label)
                        > 1
                    ):
                        class_type = "none"
                ...
                if class_type == "unpointable":
                    ...
                    if analyze:
                        if (
                            slot not in diag_seen_slots_dict
                            or value_label != diag_seen_slots_value_dict[slot]
                        ):
                            print("(%s): %s, " % (slot, value_label), end="")
                elif (
                    slot in diag_seen_slots_dict
                    and class_type == diag_seen_slots_dict[slot]
                    and class_type != "copy_value"
