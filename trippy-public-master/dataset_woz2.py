...
def get_token_pos(tok_list, label):
    ...
    found = False
    label_list = [
        item for item in map(str.strip, re.split("(\W+)", label)) if len(item) > 0
    ]
...
def get_turn_label(label, sys_utt_tok, usr_utt_tok, slot_last_occurrence):
    ...
    if label == "none" or label == "dontcare":
        ...
        in_usr, usr_pos, in_sys, _ = check_label_existence(
            label, usr_utt_tok, sys_utt_tok
        )
        if in_usr:
...
def create_examples(
    ...
    for entry in input_data:
        ...
        for turn in entry["dialogue"]:
            ...
            turn_label = {
                LABEL_FIX.get(s.strip(), s.strip()): LABEL_FIX.get(v.strip(), v.strip())
                for s, v in turn["turn_label"]
            }

            guid = "%s-%s-%s" % (
                set_type,
                str(entry["dialogue_idx"]),
                str(turn["turn_idx"]),
            )
            ...
            if delexicalize_sys_utts:
                ...
                for slot in slot_list:
                    ...
                    if label != "none" and label != "dontcare":
                        _, _, in_sys, _ = check_label_existence(
                            label, usr_utt_tok, sys_utt_tok
                        )
                        if in_sys:
            ...
            for slot in slot_list:
                ...
                (usr_utt_tok_label, class_type, is_informed) = get_turn_label(
                    label, sys_utt_tok, usr_utt_tok, slot_last_occurrence=True
                )
                ...
                if label_value_repetitions and slot in diag_seen_slots_dict:
                    if (
                        class_type == "copy_value"
                        and list(diag_seen_slots_value_dict.values()).count(label) > 1
                    ):
                        class_type = "none"
