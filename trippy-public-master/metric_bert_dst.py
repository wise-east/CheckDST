...
def check_slot_inform(value_label, inform_label, label_maps, swap_with_map=True):
    value = inform_label

    if swap_with_map == False: 
        return value
...
def get_joint_slot_correctness(fp, class_types, label_maps, slot):

    key_class_label_id = f'class_label_id_{slot}'
    key_class_prediction = f'class_prediction_{slot}'
    key_start_pos = f'start_pos_{slot}'
    key_start_prediction = f'start_prediction_{slot}'
    key_end_pos = f'end_pos_{slot}'
    key_end_prediction = f'end_prediction_{slot}'
    key_refer_id = f'refer_id_{slot}'
    key_refer_prediction = f'refer_prediction_{slot}'
    key_slot_groundtruth = f'slot_groundtruth_{slot}'
    key_slot_prediction = f'slot_prediction_{slot}'
    ...
    with open(fp) as f:
        ...
        for pred in preds:
            ...
            organized_results.append(dict_result)
        ...
        return pd.DataFrame(organized_results), np.asarray(total_correctness), np.asarray(val_correctness), np.asarray(class_correctness), np.asarray(pos_correctness), np.asarray(refer_correctness), np.asarray(confusion_matrix), c_tp, c_tn, c_fp, c_fn
...
if __name__ == "__main__":
    ...
    for fp in sorted(glob.glob(sys.argv[3])):
        ...
        for idx, slot in enumerate(slots):
            organized_result, tot_cor, joint_val_cor, cls_cor, pos_cor, ref_cor, conf_mat, ctp, ctn, cfp, cfn = get_joint_slot_correctness(fp, class_types, label_maps, slot)
            ...
            # if idx ==3 : 
            #     break 
        df_merged = reduce(lambda  left,right: pd.merge(left,right,on=['id'], how='outer'), all_results)

        df_merged.to_csv(fp.replace(".json", ".csv"), index=False)
