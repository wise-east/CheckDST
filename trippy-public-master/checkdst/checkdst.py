...
class CheckDST_Trippy:
    def __init__(self, dir_name):
        ...
        self.class_types, self.slots, self.label_maps = load_dataset_config(
            self.dataset_config
        )
    ...
    def load_tp_skip_ids(self, tp_skip_fn="data/laug_dst/TP/no_change_ids.txt"):
        ...
        with open(tp_skip_fn, "r") as f:
            self.tp_skip_ids = [
                t.lower().replace(".json", "") for t in f.read().splitlines()
            ]
    ...
    def load_need_coref_dial_ids(self, multiwoz23_fp="data/MULTIWOZ2.3/data.json"):
        ...
        for dial_id, dial in tqdm(multiwoz23.items()):
            ...
            for turn_num in range(math.ceil(len(dial["log"]) / 2)):
                ...
                if need_coref:
                    self.need_corefs.append(
                        f"{dial_id.lower()}-{turn_num}".replace(".json", "")
                    )
    ...
    def get_all_pred_files(self, file_pattern="pred_res.*.csv"):
        ...
        for inv in self.invariance_types:
            self.aug_test_fns[inv] = [
                fn for fn in all_fns if inv in fn and "test" in fn
            ]

        self.orig_test_fns = [
            fn
            for fn in all_fns
            if all([inv not in fn for inv in self.invariance_types]) and "test" in fn
        ]
        self.orig_dev_fns = [
            fn
            for fn in all_fns
            if all([inv not in fn for inv in self.invariance_types]) and "dev" in fn
        ]
    ...
    def load_pred_file_as_df(self, fn):
        if ".csv" in fn:
            ...
            logger.error(
                "Found a file that is neither a csv or a json file. Not sure how to process this file"
            )
    ...
    def get_slot_vals(self, slot, pred, joint_pd_slot, swap_with_map=True):
        ...
        elif "true" in self.class_types and turn_pd_class == self.class_types.index(
            "true"
        ):
            joint_pd_slot = "true"
        elif "false" in self.class_types and turn_pd_class == self.class_types.index(
            "false"
        ):
            joint_pd_slot = "false"
        elif "refer" in self.class_types and turn_pd_class == self.class_types.index(
            "refer"
        ):
            if pd_slot[0:3] == "§§ ":
                if pd_slot[3:] != "none":
                    joint_pd_slot = check_slot_inform(
                        joint_gt_slot, pd_slot[3:], self.label_maps, swap_with_map
                    )
            elif pd_slot[0:2] == "§§":
                if pd_slot[2:] != "none":
                    joint_pd_slot = check_slot_inform(
                        joint_gt_slot, pd_slot[2:], self.label_maps, swap_with_map
                    )
            elif pd_slot != "none":
                joint_pd_slot = pd_slot
        elif "inform" in self.class_types and turn_pd_class == self.class_types.index(
            "inform"
        ):
            if pd_slot[0:3] == "§§ ":
                if pd_slot[3:] != "none":
        ...
        if (
            ...
            and joint_gt_slot in self.label_maps
        ):
    ...
    def format_data_as_df(self, data):
        ...
        for pred in data:
            ...
            dict_result = {"id": f"{dialog_id.lower()}-{turn_idx}"}

            for slot in self.slots:
                joint_pd_slot = pred_dst.get(slot, None)

                joint_gt_slot, joint_pd_slot = self.get_slot_vals(
                    slot, pred, joint_pd_slot
                )
    ...
    def get_dial_id_to_jga_from_pred_file(self, fn):
        ...
        for idx, row in df.iterrows():
            jga = int(
                all([row[f"{slot}_gold"] == row[f"{slot}_pred"] for slot in self.slots])
            )
            if jga == 1:
            ...
            if "NEI" in fn:
                for k in row.keys():
                    if (
                        "gold" in k
                        and k.split("_")[0] in NAMED_ENTITY_SLOTS_FOR_TRIPPY
                        and row[k] != "none"
                    ):
    ...
    def load_conditional_jgas(self):
        ...
        for inv in self.invariance_types:
            ...
            for inv_fn in tqdm(self.aug_test_fns[inv]):
                ...
                if inv == "TP":
                    inv_jgas = {
                        k: v for k, v in inv_jgas.items() if k not in self.tp_skip_ids
                    }
                    orig_jgas_copy = {
                        k: v
                        for k, v in orig_jgas_copy.items()
                        if k not in self.tp_skip_ids
                    }

                if inv == "NEI":
                    # only compare those with named entity slots
                    with open(
                        f"/data/home/justincho/trippy-public-master/data/laug_dst/NEI/changed_dialids_test_{is_fewshot}.txt",
                        "r",
                    ) as f:
                        swapped_dialogs = [s.replace("\n", "") for s in f.readlines()]
                    orig_jgas_copy = {
                        k: v for k, v in orig_jgas_copy.items() if k in swapped_dialogs
                    }
                    inv_jgas = {
                        k: v for k, v in inv_jgas.items() if k in swapped_dialogs
                    }
                ...
                self.check_dst_results[step_ct][f"{inv} cJGA"] = np.mean(
                    list(conditional_jgas.values())
                )
                self.check_dst_results[step_ct][f"{inv} JGA"] = np.mean(
                    list(inv_jgas.values())
                )
    ...
    def load_coref_jgas(self):
        ...
        for step_ct, dialid2jga in tqdm(self.orig_jgas.items()):
            coref_jgas = [
                jga for dialid, jga in dialid2jga.items() if dialid in self.need_corefs
            ]
    ...
    def load_all(self):
        if os.path.isfile(self.save_path):
            logger.info(
                f"Found previously loaded results. Loading it from {self.save_path}"
            )
...
def format_dfs_for_plotting(dfs):
    ...
    grouped = grouped[
        grouped["index"] != "final"
    ]  # final is the same as the last checkpoint
    grouped["index"] = grouped["index"].apply(
        lambda x: int(x)
    )  # transform for corret ordering in plot
