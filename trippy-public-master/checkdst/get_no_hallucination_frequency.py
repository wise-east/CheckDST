...
def get_no_hall_freqs(dir):
    ...
    for pred in data:
        ...
        for slot in NAMED_ENTITY_SLOTS_FOR_TRIPPY:
            ...
            joint_gt_slot, joint_pd_slot = get_slot_vals(
                slot, pred, joint_pd_slot, swap_with_map=False
            )
    ...
    # 100% is expected
    print(
        f"no hallucination frequency: {np.mean(no_hall_freqs):.4f} (expected 100%) \t hallucination count: {count}"
    )
