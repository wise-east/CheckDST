...
def train(
    ...
    aux_sampler = (
        RandomSampler(aux_dataset)
        if args.local_rank == -1
        else DistributedSampler(aux_dataset)
    )
                ...
                if (
                    ...
                    if len(loss_diff_queue) > 0:
                        tb_writer.add_scalar(
                            "loss_diff_mean",
                            sum(loss_diff_queue) / len(loss_diff_queue),
                            global_step,
                        )
                    if aux_logged_steps > 0 and tr_aux_loss != logging_aux_loss:
                        tb_writer.add_scalar(
                            "aux_loss", tr_aux_loss - logging_aux_loss, global_step
                        )
...
def load_and_cache_examples(args, model, tokenizer, processor, evaluate=False):
    ...
    cached_file = os.path.join(
        ...
        "cached_{}_features".format(args.predict_type if evaluate else "train"),
    )
    if (
...
def load_and_cache_aux_examples(args, model, tokenizer, aux_task_def=None):
    ...
    if (
        ...
        logger.info(
            "Creating features from aux task dataset file at %s", args.mtl_data_dir
        )
        examples = processor.get_aux_task_examples(
            data_dir=args.mtl_data_dir,
            data_name=args.mtl_train_dataset,
            max_seq_length=args.max_seq_length,
        )

        features = convert_aux_examples_to_features(
            examples=examples,
            aux_task_def=aux_task_def,
            max_seq_length=args.max_seq_length,
        )
    ...
    all_start_positions = torch.tensor(
        [f.start_pos for f in features], dtype=torch.long
    )
...
def main():
    ...
    parser.add_argument(
        "--mtl_task_def", type=str, default="aux_task_def.json", help=""
    )
    parser.add_argument(
        "--mtl_train_dataset",
        type=str,
        default="",
        help="cola|mnli|mrpc|qnli|qqp|rte|sst|wnli|squad|squad-v2",
    )
    parser.add_argument(
        "--mtl_data_dir", type=str, default="data/aux/bert_base_uncased_lower", help=""
    )
    ...
    if args.do_train:
        ...
        aux_dataset, _ = load_and_cache_aux_examples(
            args, model, tokenizer, aux_task_def
        )
        global_step, tr_loss, aux_loss = train(
            args,
            train_dataset,
            aux_dataset,
            aux_task_def,
