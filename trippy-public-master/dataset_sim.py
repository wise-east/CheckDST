...
def get_tok_label(
    ...
    if slot_type not in cur_ds_dict:
        ...
        if value == "dontcare" and (
            slot_type not in prev_ds_dict or prev_ds_dict[slot_type] != "dontcare"
        ):
            ...
            if not in_usr and not in_sys:
                ...
                if slot_type not in prev_ds_dict or value != prev_ds_dict[slot_type]:
                    raise ValueError(
                        "Copy value cannot found in Dial %s Turn %s"
...
def delex_utt(utt, values, unk_token="[UNK]"):
    utt_delex = utt.copy()
    for v in values:
        utt_delex[v["start"] : v["exclusive_end"]] = [unk_token] * (
            v["exclusive_end"] - v["start"]
        )
    return utt_delex
...
def get_turn_label(
    ...
    for slot_type in slot_list:
        ...
        ]  # Don't use token labels for sys utt
            ...
            for slot in slot_list:
                ...
                if append_history:
                    if use_history_labels:
                        hst_lbl_dict[slot] = (
                            txt_a_lbl[slot] + txt_b_lbl[slot] + hst_lbl_dict[slot]
                        )
                    else:
                        hst_lbl_dict[slot] = [
                            0
                            for _ in txt_a_lbl[slot]
                            + txt_b_lbl[slot]
                            + hst_lbl_dict[slot]
                        ]
