...
class RobertaForDST(RobertaPreTrainedModel):
    def __init__(self, config):
        ...
        if self.class_aux_feats_inform:
            self.add_module(
                "inform_projection", nn.Linear(len(self.slot_list), len(self.slot_list))
            )
        if self.class_aux_feats_ds:
            self.add_module(
                "ds_projection", nn.Linear(len(self.slot_list), len(self.slot_list))
            )
        ...
        if hasattr(config, "aux_task_def"):
            self.add_module(
                "aux_out_projection",
                nn.Linear(config.hidden_size, int(config.aux_task_def["n_class"])),
            )
    ...
    def forward(
        ...
        if aux_task_def is not None:
            if aux_task_def["task_type"] == "classification":
                ...
                aux_start_loss = aux_token_loss_fct(
                    torch.cat((aux_start_logits, aux_end_logits), 1), start_pos
                )
                aux_end_loss = aux_token_loss_fct(
                    torch.cat((aux_end_logits, aux_start_logits), 1), end_pos
                )
        ...
        for slot in self.slot_list:
            if self.class_aux_feats_inform and self.class_aux_feats_ds:
                pooled_output_aux = torch.cat(
                    (
                        pooled_output,
                        self.inform_projection(inform_labels),
            ...
            if (
                class_label_id is not None
                and start_pos is not None
                and end_pos is not None
                and refer_id is not None
            ):
                # If we are on multi-GPU, split add a dimension
                ...
                token_is_referrable = torch.eq(
                    class_label_id[slot], self.refer_index
                ).float()
                ...
                if self.refer_index > -1:
                    per_example_loss = (
                        (self.class_loss_ratio) * class_loss
